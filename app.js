...
// ==========================================================
// PWA - INSTALACIÓN EN WINDOWS / ANDROID
// ==========================================================
let eventoInstalacionPWA = null;

function puedeRegistrarServiceWorker() {
  const hostLocal = ["localhost", "127.0.0.1", "[::1]"].includes(window.location.hostname);
  return "serviceWorker" in navigator && (window.location.protocol === "https:" || hostLocal);
}
...
function inicializarPWA() {
  ...
  if (estaEnModoInstalado() && btnInstalarApp) {
    btnInstalarApp.classList.add("hidden");
  }

  if (puedeRegistrarServiceWorker()) {
  ...
  window.addEventListener("beforeinstallprompt", (event) => {
    ...
    if (btnInstalarApp && !estaEnModoInstalado()) {
  ...
  if (btnInstalarApp) {
    btnInstalarApp.addEventListener("click", async () => {
      if (!eventoInstalacionPWA) {
        mostrarAlertaRiego({
          icon: "info",
          title: "Instalación no disponible todavía",
          text: "Abra la página desde HTTPS o localhost. En Android y Windows, el navegador mostrará la opción de instalar cuando la app cumpla los requisitos."
        });
        return;
      }
